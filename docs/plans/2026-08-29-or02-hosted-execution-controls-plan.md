...
5. Count every hosted dispatch. Reserve the policy's full per-request cost ceiling rather than an optimistic estimate so concurrent verified overages cannot cross the daily cap. Require a strongly consistent operation-specific reservation witness before dispatch, and after settlement require the exact reservation to be absent while same-day committed cost is monotonic; unrelated later reservations or settlements may advance the durable state without invalidating an earlier committed operation. Settle exact cost only from a verified bounded receipt; if output, receipt, timeout, cancellation, or provider completion is uncertain after dispatch, retain the reservation as a reconciliation hold so concurrency and spend capacity cannot be silently reused. Never allow a successful result whose actual hosted cost exceeds its authorized ceiling.
...
### Continuation of authorized repair loop 1 — concurrent durable state advancement

The next independent fresh-context pass found that whole-state equality after a transaction was stronger than durability and unsafe under legitimate concurrency. If request A committed a reservation or settlement and request B committed another valid transaction before A's read-back, A falsely returned `control-store-failed`; a reservation could therefore be stranded before dispatch, or a settled hosted response could be discarded after its cost was durably recorded.

The founder-directed self-correction keeps the strongly consistent `read()` requirement while replacing whole-state equality with operation-specific durable witnesses. Before hosted dispatch, the read state must contain exactly the reservation and same-minute request record bound to the request, day, scope hashes, and full authorized cost ceiling. After settlement, the exact reservation must remain absent; on the same UTC day, the request witness must remain unique and committed cost must be at least the transaction receipt's committed cost. A later day is safe only because day rollover is already impossible while that reservation is live. Fallback decisions still require a successful bounded read so a store outage cannot become fake success.

Two deterministic interleaving stores reproduce the reservation and settlement races by holding the first transaction return until the second state has committed. Both requests now complete once without a stranded reservation and exact total committed cost remains 2,500 micros. A separate unreadable-store regression proves even a kill-switch fallback fails closed before invoking the fake path. The focused OR02 contract is 17/17 and TypeScript is clean; the deterministic repository suite passed 733 tests with four intentional served-only skips, Graph v2 passed 25/25 with empty manifest errors, graph-position passed 14/14, and diff checks passed. Because OR02 never received an independent PASS, this closes another finding inside the same still-open repair attempt; it does not create or claim a second graph repair loop. No provider, route, network, production store, account mutation, money movement, UI, persistence, or protected/frozen path enters this bounded correction.
