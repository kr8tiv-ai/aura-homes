...
The next adversarial pass on that same unclosed repair proved the complementary compile-success case: changing explicit opening priorities can produce a different valid canonical project while stale rehashed geometry still satisfies counts, head fit, and unchanged disclosure arrays. The validator now preserves the specific program/opening/climate refusals first, then requires the accepted proposal hash to equal the successful canonical recompile. A south/west-to-north/east placement regression proves that compiler-owned document geometry cannot drift even when both projects compile. This is the final correction within repair-loop use `1`; it changes no authority, dependency, side effect, or write-set boundary.
