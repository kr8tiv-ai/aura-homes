...
| `npm test` (app) — deterministic app + contract-truth specs | 735 declared (731 passed + 4 served-only skipped) |
| `npm run test:ui` (app) — Playwright against a fresh static export | 137 declared (136 passed; frozen reduced-motion walkthrough timing gate blocks UX07 closure) |
