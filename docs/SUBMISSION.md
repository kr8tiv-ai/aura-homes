...
| Guided and Pro editors over one `BuilderDocument`, 55-plan library | **Live** | [/build](https://aurahomes.fun/build) — every plan names its source and licence |
...
| 18–36s | Design | `/build?mode=guided` — the 55-plan library, pick the Fjell Cube, the camera reframes, one Ctrl-K phrase edit ("wider by 4") lands as one undo step. |
...
2. "The plan library hit 55 — including three 1960s USDA A-frames reborn
...
| `npm test` (app) — deterministic app + contract-truth specs | 454 passed |
