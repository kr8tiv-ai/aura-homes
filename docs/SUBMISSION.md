...
| `npm test` (app) — deterministic app + contract-truth specs | 736 declared (732 passed + 4 served-only skipped) |
| `npm run test:ui` (app) — Playwright against a fresh static export | 137/137 passed at source `49d41019`; the UX07 production, served-spec, and UI-configuration blobs remain unchanged from that source-bound receipt. |
