...
**On-ramp (Canadian):** Wealthsimple sells USDC at 0% trading fee (its only free pair) → withdraw on Base → **Circle CCTP** burn-and-mint to X Layer native USDC. Kraken (USDC/CAD spot, free Interac deposits) and Coinbase Canada are the alternates. This is a 2-hop UX today; the app wraps it in guided steps, and account abstraction + a fiat on-ramp partner are the roadmap fix.
...
| Contingency 10/12/15% | ~$17.8K | ~$31.7K | ~$56.7K |
| **Total ex-land** (computed: Σ lines × contingency) | **~$195K** | **~$296K** | **~$435K** |
| **Total with land** | **~$270K** | **~$446K** | **~$785K** |
