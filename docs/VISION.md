...
6. **Off-grid everything, grid-optional** — solar power, AWG in the water plan *(honestly positioned: summer supplement; cistern/well primary — physics is physics)*, off-grid sewage and greywater handled eco-first, wood stove heat. Every home connectable to grid later.
...
8. **One-click feel** — answer questions, get a designed home, fund it, watch it build. Normies must be able to use it: no visible crypto complexity, prices readable in CAD, token (if ever) invisible behind the scenes.
