...
6. **Off-grid everything, grid-optional** — solar power, an AWG unit **recommended on every home** *(plumbed into the water loop as the summer producer; cistern/well carries winter — physics is physics; the reference configuration still includes it)*, off-grid sewage and greywater handled eco-first, wood stove heat. Every home connectable to grid later.
...
12. **A KR8TIV AI product** — part of the family: same aura mark (recolored aurora-over-forest, Banff energy), same premium design language *(ground pivoted dark → light-first Aug 9, 2026, the founder's call — paper white with the aurora inside the mark and framed media; see BRAND.md v3)*, its own distinct identity. Branding "on a level with Tesla, Apple." No AI slop.
